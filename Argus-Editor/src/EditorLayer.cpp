...
#include "Argus/Scene/SceneSerializer.h"
#include "Argus/Utils/PlatformUtils.h"

#include "ImGuizmo.h"
#include "Argus/Math/Math.h"
...
namespace Argus {
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		{
			{
				auto view = m_ActiveScene->GetAllEntitiesWith<TransformComponent, BoxCollider2DComponent>(); // box colliders
			...
			{
				auto view = m_ActiveScene->GetAllEntitiesWith<TransformComponent, CircleCollider2DComponent>(); // circle colliders
				for (auto entity : view)
				{
	...
	{
		std::string filepath = FileDialogs::OpenFile("Argus Scene (*.argus)\0*.argus\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".argus")
		{
	...
	{
		std::string filepath = FileDialogs::SaveFile("Argus Scene (*.argus)\0*.argus\0");
		if (!filepath.empty())
		{
